...
class WhatsAppBotApi {
  ...
  }) : whats_app_crypto = crypto ?? Crypto.defaultCrypto() {
    if (httpClient != null) {
  ...
  WaClientData waClientData({
    ...
    try {
      ...
      Map decyprt = json.decode(whats_app_crypto.decrypt(data: (query["wa"] as String).replaceAll(RegExp("([ ])"), "")));
      return WaClientData(decyprt);
    } catch (e) {
      if (query["wa-client"] is String == false) {
        query["wa-client"] = "";
      }
      Map decyprt = json.decode(whats_app_crypto.decrypt(data: (query["wa-client"] as String).replaceAll(RegExp("([ ])"), "")));
      return WaClientData(decyprt);
    }
  }
